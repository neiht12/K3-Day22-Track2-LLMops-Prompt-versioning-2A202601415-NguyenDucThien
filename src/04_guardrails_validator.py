...
# This local demo does not need Guardrails telemetry. Disable it before importing
# Guardrails so an unavailable proxy/network cannot produce distracting errors.
...
def configure_utf8_console() -> None:
    """Use UTF-8 for Vietnamese output in legacy Windows terminals."""
...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        if found_pii:
            ...
            return FailResult(
                error_message=f"Phát hiện PII: {', '.join(pii_type for pii_type, _ in found_pii)}",
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        try:
            ...
            # TODO: Trả về PassResult với json.dumps(parsed, indent=2)
            return FailResult(
                error_message="JSON đã được định dạng lại",
...
def demo_json_guard():
    ...
    for label, text in test_cases:
        ...
        # Guardrails marks a value fixed by OnFailAction.FIX as validation-passed.
        # Label the fallback separately so the demo accurately shows that this
        # input could not be repaired into the user's intended JSON.
        try:
            json.loads(text)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run the Step 4 Guardrails validator demonstrations."
    )
    demo_group = parser.add_mutually_exclusive_group()
    demo_group.add_argument(
        "--pii-only",
        action="store_true",
        help="Run only the PII redaction demonstration.",
    )
    demo_group.add_argument(
        "--json-only",
        action="store_true",
        help="Run only the JSON repair demonstration.",
    )
